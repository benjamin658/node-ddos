    ...
    q.series([
            function (lib) {
                request.get({url: 'http://localhost:5050/article', json: true}, function (err, resp, body) {
                    ...
                    t.deepEqual(ddos.table[key], {count: 1, expiry: 1, alert: false})
                    lib.done()
                })
            },
            function (lib) {
                request.get({url: 'http://localhost:5050/article', json: true}, function (err, resp, body) {
                    var key = Object.keys(ddos.table)[0]
                    t.deepEqual(ddos.table[key], {count: 2, expiry: 1, alert: false})
                    lib.done()
                })
            },
            function (lib) {
                request.get({url: 'http://localhost:5050/article', json: true}, function (err, resp, body) {
                    var key = Object.keys(ddos.table)[0]
                    t.deepEqual(ddos.table[key], {count: 3, expiry: 1, alert: false})
                    lib.done()
                })
            },
            function (lib) {
                request.get({url: 'http://localhost:5050/article', json: true}, function (err, resp, body) {
                    var key = Object.keys(ddos.table)[0]
                    t.deepEqual(ddos.table[key], {count: 4, expiry: 2, alert: false})
                    lib.done()
                })
            },
            function (lib) {
                request.get({url: 'http://localhost:5050/article', json: true}, function (err, resp, body) {
                    var key = Object.keys(ddos.table)[0]
                    t.deepEqual(ddos.table[key], {count: 5, expiry: 4, alert: false})
...
tape('then', function (t) {
    ...
    setTimeout(function () {
        request.get({url: 'http://localhost:5050/article', json: true}, function (err, resp, body) {
            ...
            // expiry goes to 2 
            t.deepEqual(ddos.table[key], {count: 6, expiry: 2, alert: false})
        })
    }, 3100)
})
